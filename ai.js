...
function addCodeCopyButtons(container){
  container.querySelectorAll("pre").forEach(pre=>{
    ...
    btn.onclick=()=>{navigator.clipboard.writeText(code.innerText).then(()=>{
...
async function sendMessage(isRegenerate=false){
  ...
  if(!isRegenerate){
    ...
    if(pendingAttachments.length){
      const loadedFiles=await Promise.all(pendingAttachments.map(f=>new Promise(res=>{
        const reader=new FileReader();
        reader.onload=()=>{res({inlineData:{mimeType:f.type||"application/octet-stream",data:reader.result.split(",")[1]}});};
  ...
  try{
    const res=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents,generationConfig:{temperature:0.7}})});
...
fileInput.addEventListener("change",()=>{pendingAttachments.push(...Array.from(fileInput.files));fileInput.value="";});
