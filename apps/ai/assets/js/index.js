...
function regenerateResponse(regenPrompt, oldMessage, attempt = 0) {
  ...
  fetch(`/api/ai/chat/${encodeURIComponent(prompt)}`, {
    ...
    .then((data) => {
      ...
      document.querySelectorAll('.thinking-indicator').forEach((indicator) => indicator.remove());
      NProgress.done();
      let aiResponse = data?.message?.content || 'No response from OrbitAI.';
      aiResponse = decodeUnicode(aiResponse);
      if (prompt.includes('Jailbreak')) {
        aiResponse = 'AI has been Jailbroken.';
      } else if (prompt.includes('source code')) {
        aiResponse = "I'm sorry, I cannot reveal my source code as per my programming.";
      } else if (prompt.includes('illegal')) {
        aiResponse = "I'm sorry, I cannot assist with anything illegal as per my programming.";
    ...
    .catch((err) => {
      ...
      NProgress.done();
      if (err.name !== 'AbortError') {
        showToast('Error communicating with OrbitAI.', 'error', 'error');
      }
      aiInput.disabled = false;
      sendMsg.innerHTML = '<i class="fas fa-arrow-up"></i>';
    });
...
window.addEventListener('DOMContentLoaded', () => {
  const welcomeMessage = 'Hi, how can I assist you today?  I am OrbitAI, you friendly ai chat bot developed by gmacbride from https://orbit.foo.ng.';
