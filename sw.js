...
importScripts('https://ptza.env.pm/scram/scramjet.all.js');
...
async function handleRequest(event) {
  ...
  if (url.hostname.includes('youtube.com') || url.hostname.includes('youtube-nocookie.com')) {
    const redirectUrl = `https://ptza.env.pm/api/youtube-bypass/embed.html#${url.href}`;
...
scramjet.addEventListener('request', (e) => {
  if (playgroundData && e.url.href.startsWith(playgroundData.origin)) {
    ...
    } else if (e.url.href === origin + 'https://ptza.env.pm/style.css') {
      headers['content-type'] = 'text/css';
      e.response = new Response(playgroundData.css, { headers });
    } else if (e.url.href === origin + 'https://ptza.env.pm/script.js') {
