  ...
  async function setupScrapePlayer(videoId) {
    ...
    try {
      ...
      if (!videoUrl || typeof videoUrl !== 'string' || !videoUrl.startsWith("http")) {
        throw new Error("Invalid video URL received. This video may be restricted or unavailable for scraping.");
      }
      ...
      plyrInstance.on("ready", () => {
        ...
        if (controls) {
          ...
          downloadBtn.innerHTML = `
            <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" style="fill:currentColor; width: 18px; height: 18px;">
      ...
      plyrInstance.on("error", () => {
        throw new Error("Failed to play video. The format might not be supported.");
      });
    } catch (error) {
      rippleLoader.style.display = "none";
      rippleLoader.classList.remove("active");
